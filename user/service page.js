...
async function getServices() {
    ...
    form.append("s_id", document.getElementById("serviceStationSelection").value);
    let res = await fetch("userAll.php", {
        method: "POST",
        body: form
    });
...
async function findServiceStation() {
    let service = document.getElementById("search").value;
...
async function addServiceByStation() {
    ...
    let serviceId = document.getElementById("availableServices").value;
...
function showUserRegServices() {
    ...
    fetch("userAll.php", {
        ...
        let details = JSON.parse(textData);
        pending.empty();
        done.empty();
        pending.append(head);
        done.append(head);
...
async function cancelRegRequest(reqId) {
    let form = new FormData();
