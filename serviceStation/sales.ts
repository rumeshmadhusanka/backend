...
async function getTotalEarnings() {
    ...
    (<HTMLElement>document.getElementById("totalEarnings")).innerHTML = `Total Earnings:LKR: ${tot}`;
}
...
async function getTopCustomers() {
    ...
    let text = await res.json();
    (<HTMLElement>document.getElementById("topCustomers")).innerHTML=`<tr>
            <td>Customer Name</td>
            <td>Mobile Number</td>
            <td>Purchases</td>
        </tr>`;
    for (let i = 0; i <text.length ; i++) {
        ...
        (<HTMLElement>document.getElementById("topCustomers")).innerHTML+=template;
    }
